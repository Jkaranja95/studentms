...
if (count($sqlArr)) {
  $count_sql = "SELECT * FROM `finance` WHERE $sql ORDER BY `id` DESC";
  $result_sql = "SELECT * FROM `finance` WHERE $sql ORDER BY `id` DESC LIMIT $offset , $records_no_per_page";
} else {
  $count_sql = "SELECT * FROM `finance` WHERE 1 ORDER BY `id` DESC";
  $result_sql = "SELECT * FROM `finance` WHERE 1 ORDER BY `id` DESC LIMIT $offset , $records_no_per_page ";
}
        ...
        <div class="col-lg-12">
          <div class="panel panel-default">
            <div class="panel-heading">
              <i class="fa fa-th-list fa-fw"></i>Finance
              <span class="pull-right" style="margin-top: -7px;">
                <a id="add_property_button" type="button" name="button" class="btn btn-success" href="add_finance.php">
            ...
            <div class="panel-body">
              ...
              <div class="table-responsive" id="reportData">
                <table class="table table-striped table-bordered table-hover">
                  <thead>
                    <tr class="info">
                      <th colspan="15" class="text-center"> Finance</th>
                    </tr>
                    <tr>
                      <th>Date</th>
                      <th>Adm No</th>
                  ...
                  <tbody>
                    ...
                    foreach ($table_rows as $row) { ?>
                      <tr>
                        ...
                        $conn = Db::get()->conn;
                        $stmt = $conn->prepare($sql);
                        $stmt->bindparam("admno", $row->admno);
                        $stmt->bindparam("form", $row->form);
                        $stmt->bindparam("term", $row->term);
                        ...
                        <td>
                          <a href="detail_finance.php?finance_id=<?php echo $row->id; ?>"
